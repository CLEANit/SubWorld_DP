...
def save_policy(seed, path, n_steps, n_t, uncert_pos, n_h, size, gps_cost, cur_cost, uncert_cur, max_cur, uncert_res):
    pos, pos_est, i, no_gps, no_cur, status = policy_gps(path, seed, None, None, n_steps, n_t, uncert_pos, n_h, size, gps_cost, cur_cost, uncert_cur, max_cur, uncert_res)
...
if __name__ == "__main__":
    ...
    pool.starmap(save_policy, zip(seeds, repeat(path), repeat(n_steps), repeat(n_t), repeat(uncert_pos), repeat(n_h), repeat(size), repeat(gps_cost), repeat(cur_cost), repeat(uncert_cur), repeat(max_cur), repeat(uncert_res)))
    pool.close()
    pool.join()
